    ...
    <nav className="hidden lg:flex lg:items-center bg-[#F9FAFB] dark:bg-white/3 rounded-full p-1 max-h-fit">
      {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                {
                  "bg-white dark:bg-white/5 text-gray-900 dark:text-white font-medium shadow-xs":
                    isActive,
                  "hover:text-primary-600 dark:hover:text-primary-400 hover:bg-white/50 dark:hover:bg-white/5":
                    !isActive,
                },
              )}
            >
        ...
        if (item.type === "dropdown") {
          ...
          const hasActiveChild = item.items.some(({ href }) =>
          ...
          return (
            ...
            >
              ...
              {isDropdownActive && (
                <div
                  className="absolute left-0 mt-2 w-50 bg-white dark:bg-gray-900 rounded-xl shadow-theme-xl border border-gray-100 dark:border-gray-800 p-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200"
                  onMouseEnter={() => openDropdown(item.label)}
                  onMouseLeave={() => closeDropdown()}
                >
                              ...
                              className={cn(
                                "p-1.5 rounded-lg mt-0.5 shrink-0",
                                {
                                  "bg-primary-100 dark:bg-primary-900/30 text-primary-600 dark:text-primary-400":
                                    isSubActive,
                                  "bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 group-hover:bg-primary-50 group-hover:text-primary-600":
                                    !isSubActive,
                                },
                              )}
                            >
