...
// New interface for enhanced stats API response
// interface EnhancedSMSStats {
//   totals: {
//     sent: number;
//     failed: number;
//     delivered: number;
...
export default function DashboardHome() {
  ...
  const actions = [
    ...
    },
    {
      ...
      onClick: () => router.push("/otp/overview"),
      variant: "outline",
    },
    {
  ...
  //         },
  //       }),
  //       // FIXED: Use the new reports endpoint for SMS stats
  //       fetch("/api/reports/sms-stats?period=all", {
  //         headers: {
  //           Authorization: `Bearer ${token}`,
  //         },
  //       }),
  ...
  //     // Handle SMS stats response - UPDATED for new API structure
  //     if (statsResponse.ok) {
  //       const statsData = await statsResponse.json();
  //       console.log("Enhanced SMS Stats Response:", statsData);

  //       let transformedStats: SMSStats;

  //       // Check if this is the new enhanced stats API structure
  //       if (statsData.data?.totals?.sent !== undefined) {
  //         // New API structure
  //         transformedStats = {
  //           totals: {
  //             messages: statsData.data.totals.sent || 0,
  //             cost: statsData.data.totals.cost || 0,
  //             averageCost: statsData.data.totals.averageCost || 0,
  //           },
  //           // eslint-disable-next-line @typescript-eslint/no-explicit-any
  //           byStatus:
  //             statsData.data.byStatus?.map((statusItem: any) => ({
  //               status: statusItem.status,
  //               _count: { id: statusItem.count },
  //               _sum: { cost: 0 }, // Cost data might not be available in byStatus
  //             })) || [],
  //           timeline: [],
  //           topRecipients: [],
  //         };
  ...
  //         transformedStats = statsData.data;
  //       }

  //       setSmsStats(transformedStats);
  //     } else {
  //       console.error("Failed to fetch SMS stats:", statsResponse.status);
  //       // Set fallback stats if API fails
  //       const fallbackStats: SMSStats = {
  //         totals: {
  //           messages: 0,
  //           cost: 0,
  //           averageCost: 0,
  ...
  //         topRecipients: [],
  //       };
  //       setSmsStats(fallbackStats);
  //     }

  //     // Handle SMS history response
  //     if (historyResponse.ok) {
  //       const historyData = await historyResponse.json();
  //       console.log("SMS History API Response:", historyData);

  //       // Transform the data to ensure consistent sender name
  //       const transformedHistory =
  //         historyData.data?.map((sms: SMSHistory) => {
  //           // Handle both senderName and sender object formats
  //           let senderName = sms.senderName;

  //           if (!senderName && sms.sender) {
  //             // If sender object exists, use its name
  //             senderName =
  //               typeof sms.sender === "object" ? sms.sender.name : sms.sender;
  //           }

  //           if (!senderName) {
  //             // Fallback to senderId if no name available
  //             senderName = sms.senderId || "N/A";
  //           }
  ...
  //     } else {
  //       console.error("Failed to fetch SMS history:", historyResponse.status);
  //       setSmsHistory([]);
  //     }
  ...
  //     setCreditBalance({ SMS: 0, WALLET: 0 });
  //     setSmsStats({
  //       totals: { messages: 0, cost: 0, averageCost: 0 },
  //       byStatus: [],
  //       timeline: [],
  //       topRecipients: [],
  //     });
  ...
  const getSenderName = (sms: SMSHistory): string => {
    return sms.senderName || sms.sender?.name || sms.senderId || "N/A";
  };
            ...
            <TableBody>
              {smsHistory.map((sms) => (
                <TableRow key={sms.id}>
                  ...
                  <TableCell>
                    <Badge variant="outline">{getSenderName(sms)}</Badge>
                  </TableCell>
          ...
          <CardFooter className="flex items-center justify-between border-t px-6 py-4">
            ...
            <div className="space-x-2">
              <Button variant="outline" size="sm" asChild>
                <Link href="/reports/sms/history">View All</Link>
              </Button>
            </div>
          </CardFooter>
        )}
